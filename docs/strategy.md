...
3. [시나리오](#3-시나리오)
   ...
   - [3단계: 정상 파일의 메타데이터에 악성 코드를 넣어 업로드한다면?](#-3단계-정상-파일의-메타데이터에-악성-코드를-넣어-업로드한다면)
   - [4단계: 압축 파일 내부에 악성 파일을 숨겨 업로드한다면?](#-4단계-압축-파일-내부에-악성-파일을-숨겨-업로드한다면)
...
### 📌 3단계: 정상 파일의 메타데이터에 악성 코드를 넣어 업로드한다면?
...
### 📌 4단계: 압축 파일 내부에 악성 파일을 숨겨 업로드한다면?

**👉 압축 파일을 임시 해제하여 내부 파일을 재귀적으로 검증하고, Zip Bomb는 압축률로 차단합니다. (또는 서버에서 압축 해제 금지)**
...
**방어 방법 3: 서버에서 압축 해제 금지**
- 압축 파일은 다운로드만 가능하도록 설정
- **서버에서 절대 압축 해제하지 않음**
- 사용자가 로컬에서 해제하여 책임 전가
